...
void vApplicationIdleHook()
{
    ...
    for(i=1;i<=configNumberOfTasks;i++){
        globalBehaviourTable[i][0] = 3; // Normal Behaviour is assigned for all tasks at the idle instant
        globalRuntimeBudgetTable[i][0] = globalBudgetTable[i][0]; // Normal Budget is assigned for all tasks at the idle instant
    }
}
